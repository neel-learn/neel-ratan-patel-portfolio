...
const Contacts = () => {
  ...
  const currentUserLiked = useSelector((state) => state.portfolio.currentUserLiked);
      ...
      <div className="contact-container">
        ...
        <div className="contact-local-container">
          {socialLinks.map((link, index) => (
            <a key={index} href={link.href} className="contact-local-elements" id={link.id} target="_blank" rel="noreferrer">
          ...
          >
            <span id="likeBtn" aria-pressed={liked}>
              {/* FIXED: Changed class to className and style string to object */}
              {liked ? (
                <i className="fa-solid fa-heart" style={{ color: "#ed0707", fontSize: "1.4rem" }}></i>
              ) : (
                <i className="fa-regular fa-heart" style={{ color: "#151313ff", fontSize: "1.4rem" }}></i>
              )}
            </span>
            <span id="likeText" style={{ fontSize: "10px" }}>
              {likeCount} {liked ? 'liked' : 'likes'}
            </span>
          </section>
        </div>
